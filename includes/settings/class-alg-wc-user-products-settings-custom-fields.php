<?php
/**
 * ZILI User Products for WooCommerce - Custom Fields Section Settings
 *
 * @version 2.0.1
 * @since   1.5.0
 *
...
class Alg_WC_User_Products_Settings_Custom_Fields extends Alg_WC_User_Products_Settings_Section {
	...
	function __construct() {
		...
		$this->desc = __( 'Custom Fields', 'zili-user-products-for-woocommerce' );
		parent::__construct();
	}
	...
	function get_settings() {
		$settings = array(
			array(
				'title' => __( 'Custom Fields Options', 'zili-user-products-for-woocommerce' ),
				'type'  => 'title',
				'id'    => 'alg_wc_user_products_custom_fields_options',
			),
			array(
				'title'             => __( 'Total custom fields', 'zili-user-products-for-woocommerce' ),
		...
		for ( $i = 1; $i <= get_option( 'alg_wc_user_products_custom_fields_total', 1 ); $i++ ) {
			$settings = array_merge( $settings, array(
				array(
					'title'   => __( 'Custom field', 'zili-user-products-for-woocommerce' ) . ' #' . $i,
				...
				array(
					'desc'    => __( 'Meta key', 'zili-user-products-for-woocommerce' ),
